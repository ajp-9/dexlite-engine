...
namespace dex
{
    namespace Component
    {
        struct Transform
        {
            ...
            void moveByLocal(const glm::vec3& amount)        { m_Position += (amount.x * getRight()) + (amount.y * getUp()) + (amount.z * getForward()); m_FlagChanged = true; }
            ...
            BasicTransform getBasicTransform() { return { m_Position, m_Orientation }; }
